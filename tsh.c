...
void eval(char *cmdline) 
{
    ...
    strcpy(buf,cmdline);
    #if DEBUG
    printf("debug message: begin to call parseline() from eval\n");
    #endif
    ...
    #if DEBUG
    printf("debug message: eval finished\n");
    #endif
    return;
}
...
int builtin_cmd(char **argv) 
{
    ...
    if (!strcmp(argv[0], "bg") || !strcmp(argv[0], "fg")){
        #if FBG_DEBUG
        printf("perform do_bgfg() in builtin_cmd\n");
        #endif
        do_bgfg(argv);
        return 1;
    }
...
void sigchld_handler(int sig) 
{
    ...
    while ((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0){
        ...
        if (WIFEXITED(status)){
            ...
            deletejob(jobs, pid);
            #if DEBUG
            printf("delete job by sigint in sihchld handler\n");
            #endif    
...
{
    #if DEBUG
    printf("signit received\n");
    #endif
    int pid = fgpid(jobs);
    if (pid > 0){
        if (kill(-pid,sig) < 0)
            unix_error("siginit error\n");
    }
    return;
}
